"""
Meta-Llama Maverick + LangChain Prompt-Based AI Agent
Tool'lar LLM ile prompt engineering entegrasyonu ile çalışır
"""
...
class LLMAgent:
    ...
    def _initialize_systems(self):
        """Groq ve LangChain sistemlerini başlat"""
        ...
        # LangChain + Groq LLM (tool entegrasyonu için)
        self.langchain_llm = ChatGroq(
            groq_api_key=self.settings.groq_api_key,
            model_name=self.settings.tool_model,  # Tool kulımı için optimize
        ...
        logger.info(f"🦙 Meta-Llama Maverick + LangChain Agent başlatıldı")
        logger.info(f"📝 Text Model: {self.current_text_model}")
        logger.info(f"👁️ Vision Model: {self.current_vision_model}")
        logger.info(f"🛠️ Tool Model: {self.settings.tool_model}")
    ...
    def switch_model(self, text_model: str, vision_model: str):
        ...
        return f"✅ Meta-Llama Maverick Modeller güncellendi!\n📝 Text: {text_model}\n👁️ Vision: {vision_model}"
    
    def process_message(self, message: str, image=None) -> str:
        """Meta-Llama Maverick ile mesajı işle - LangChain + prompt-based tool entegrasyonu"""
        try:
            ...
            error_msg = f"❌ Meta-Llama Maverick işlem hatası: {str(e)}"
